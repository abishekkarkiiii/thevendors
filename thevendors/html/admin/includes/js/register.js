  ...
  (function () {
    ...
    if (typeof stepsValidation !== undefined && stepsValidation !== null) {
      ...
      const multiSteps3 = FormValidation.formValidation(stepsValidationFormStep3, {
        ...
        var formData = {};
        console.log(document.getElementById('multiStepsCountry'));

        // Send AJAX request
        $.post(
          '../admin/includes/php/create_account.php',
          {
            multiStepsUsername: $('#multiStepsUsername').val(),
            multiStepsEmail: $('#multiStepsEmail').val(),
            multiStepsPass: $('#multiStepsPass').val(),
            multiStepsCompany: $('#multiStepsCompany').val(),
            ...
            multiStepsPincode: $('#multiStepsPincode').val(),
            multiStepsAddress: $('#multiStepsAddress').val(),
            multiStepsArea: $('#multiStepsArea').val(),
            multiStepsCity: $('#multiStepsCity').val(),
            country: $('#multiStepsCountry').val(),
            customRadioIcon: register.customRadioIcon.value
          },
          function (data, status) {
            if (data == 1) {
              window.location.href = 'http://' + 'localhost' + '/thevendors/html/admin/login.php';
            } else if (data == 2) {
              window.location.href = 'http://' + 'localhost' + '/thevendors/html/admin/register.php?err';
            } else if (data == 3) {
              window.location.href = 'http://' + 'localhost' + '/thevendors/html/admin/register.php?notallowed';
            }
          }
        );
      });
