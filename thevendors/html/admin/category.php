...
<body>
                            ...
                            <div class="card-datatable table-responsive pt-0">
                                ...
                                <table class="datatable-custom table ">
                                    ...
                                    <tbody>
                                        ...
                                        $zone = mysqli_query($company_db, "select * from category");
                                        while ($r = mysqli_fetch_assoc($zone)) {
                                            $pro_count = mysqli_query($company_db, "select * from products where category_id=" . $r['category_id']);
                                        ?>
                                            <tr>
                                                <td><?= $r['category_id'] ?></td>
                                                <td><?= $r['category_name'] ?></td>
                                                <td><?php echo mysqli_num_rows($pro_count) ?></td>
                                                ...
                                                <td><button data-bs-target="#editModal" onclick="get_zone_edit(this.value)" value="<?= $r['category_id'] ?>" data-bs-toggle="modal" class="btn btn-primary"><i class="ri-edit-line"></i></button></td>
                                            </tr>
                                        <?php
                                        }
                                        ?>
            ...
            <div class="modal-content">
                ...
                <form action="includes/php/functions_company.php" method="post">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" name="create_cat" class="btn btn-primary">Save</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                <form action="includes/php/functions_company.php" method="post">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col mb-6 mt-2">
                                <div class="form-floating form-floating-outline">
                                    <input type="text" id="Category_edit_name" name="Category_edit_name" class="form-control" placeholder="Enter Zone Name" />
                                    <label for="Category_edit_name">Category name</label>
                                </div>
                                <div class="mt-2">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" name="edit_cat" class="btn btn-primary">Save</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function get_zone_edit(x) {
            $.ajax({
                url: 'includes/php/functions_ajax.php?fun=get_cat_data&val=' + x,
                type: 'GET',
                success: function(res) {
                    res = JSON.parse(res)
                    // console.log(res);
                    document.getElementById('Category_edit_name').value = res[0].category_name;
                    document.getElementById('id_hide').value = res[0].category_id;
                    if (res[0].status == 1) {
